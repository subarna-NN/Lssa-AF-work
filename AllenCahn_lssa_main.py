...
def compute_fdm(Nx=512, Nt=5000, eps2=EPS2):
    print(f"[FDM] IMEX Allen-Cahn Nx={Nx}, Nt={Nt} ...")
    t0=time.time()
    ...
    print(f"      done ({time.time()-t0:.1f}s) | {ptr} snapshots")
...
class LSSA_PINN_AllenCahn(nn.Module):
...
def allencahn_loss(model,xc,tc,xb,tb,xi,ti,ui,eps2=EPS2):
    ...
    return 1.0*lp+10.0*lb+20.0*li, lp.item(), lb.item(), li.item()
...
def sample_points():
    ...
    xi_int=np.concatenate([np.random.uniform(-1.,-0.6,N_interface//2),
                            np.random.uniform(0.6,1.,N_interface//2)])
    ti_int=np.random.uniform(0.3,1.,N_interface)
    xc=np.concatenate([xc,xi_int]); tc=np.concatenate([tc,ti_int])
    xb=np.full(N_bc,-1.0); tb=np.random.uniform(0,1,N_bc)
    xi=np.random.uniform(-1,1,N_ic); ti=np.zeros(N_ic)
...
def train(model):
    model.to(DEVICE)
    xc,tc,xb,tb,xi,ti,ui=sample_points()
    ...
    for rnd in range(4):
        ...
        def closure():
            opt_lb.zero_grad()
            loss,_,_,_=allencahn_loss(model,xc,tc,xb,tb,xi,ti,ui)
...
model=LSSA_PINN_AllenCahn(); info=train(model)
print("Evaluating..."); U_pred=evaluate(model,x_fdm,t_fdm)
...
err=np.abs(U_pred-U_ref); vmin,vmax=U_ref.min(),U_ref.max()
...
for ax,(data,title) in zip(axes[:2],
    [(U_ref,'FDM Reference'),
     (U_pred,'LSSA-PINN Prediction')]):
    c=ax.contourf(X,T_,data,levels=120,cmap='RdBu_r',vmin=vmin,vmax=vmax)
    cb=fig.colorbar(c,ax=ax,shrink=0.90,pad=0.02)
...
cb3=fig.colorbar(c3,ax=axes[2],shrink=0.90,pad=0.02,format='%.2e')
...
styles_fdm=['--',':','-.']
for st,col,ls in zip([0.25,0.50,1.00],colors_s,styles_fdm):
...
fig,ax=plt.subplots(figsize=(7,5.5))
fig.patch.set_facecolor('white'); ax.set_facecolor('white')
ax.hist(betas,bins=40,color='#E67E22',edgecolor='white',linewidth=0.6,alpha=0.90)
ax.axvline(float(np.mean(betas)),color='#784212',lw=2.5,ls='--',
           label=f'Mean $\\bar\\beta = {float(np.mean(betas)):.3f}$')
ax.axvline(1.0,color='#7F8C8D',lw=1.8,ls=':',label='Init $\\beta_0 = 1.0$')
ax.set_xlabel('$\\beta$',fontsize=13); ax.set_ylabel('Count',fontsize=13)
ax.set_title('',fontweight='bold',fontsize=14,pad=10)
ax.legend(fontsize=11,framealpha=0.9)
ax.grid(True,linestyle='--',linewidth=0.6,alpha=0.45)
ax.tick_params(labelsize=11)
